...
describe("Given I am connected as an employee", () => {
  describe("When I am on Bills Page", () => {
    ...
    test("Then it should format the bills correctly", async () => {
      ...
      expect(bills[0].date).toBe("4 Déc. 24"); // Formatage réussi
      expect(bills[0].status).toBe("En attente"); // Statut formaté
      expect(bills[1].date).toBe("not-a-date"); // Donnée brute conservée
      expect(bills[1].status).toBe("Accepté"); // Statut formaté
    });
